        ...
        <>
          <div className="flex items-center justify-between rounded-sm bg-emerald-50 border border-emerald-200 px-4 py-3">
            <div className="flex items-center gap-4 text-sm flex-wrap">
              ...
              <span className="text-text-secondary">العميل: <strong>{(detail || ret).customer_name || "—"}</strong></span>
          ...
          {detail && (detail.refund_method === "split" ? (
            <div className="rounded-sm border border-border-normal bg-bg-overlay px-4 py-3 space-y-1.5 text-2sm">
              <p className="text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">تفاصيل طريقة الاسترداد</p>
            ...
            <div className="rounded-sm border border-border-normal bg-bg-overlay px-4 py-3 space-y-1.5 text-2sm">
              <p className="text-[11px] font-black text-text-muted uppercase tracking-widest mb-2">طريقة الاسترداد</p>
              <div className="flex justify-between items-center">
                <span className="text-text-secondary">نقداً</span>
...
export default function SalesReturnTodayModal({ open, onClose, onNavigate: propNavigate, initialFilters }) {
  ...
  const docColumns = [
    ...
    { id: "original_invoice_no", header: "الفاتورة الأصلية", width: 148, sortable: false,
      headerClass: "text-center px-2 font-black uppercase tracking-widest text-text-muted",
      cellClass: "px-2 font-mono text-[11px] font-bold text-text-secondary text-center",
      render: (r) => r.original_invoice_no ? r.original_invoice_no : <span className="inline-flex items-center rounded-sm border px-1.5 py-0.5 text-[11px] font-black bg-warning-bg text-warning-text border-warning-border">مباشر</span> },
    { id: "total", header: "الإجمالي", width: 110, sortable: true,
      headerClass: "text-center px-2 font-black uppercase tracking-widest text-text-muted",
      cellClass: "px-2 number-fmt-primary text-sm text-success-text text-center",
      render: (r) => formatMoney(r.total) },
    { id: "refund_method", header: "طريقة الرد", width: 155, sortable: true,
      headerClass: "text-center px-2 font-black uppercase tracking-widest text-text-muted",
      cellClass: "px-2",
      render: (r) => {
          ...
          <div className="flex flex-col gap-0.5 items-center">
            ...
            {isSplit && (
              <div className="flex flex-col gap-0.5 items-center">
                ...
                <span className="text-[11px] font-bold text-info-text">رصيد: {formatMoney(creditAmt)}</span>
              </div>
            )}
          </div>
        );
    ...
    { id: "actions", header: "", width: 70, headerClass: "px-2", cellClass: "px-2",
      render: (r) => (
        <div className="flex gap-1 justify-center">
          <button onClick={() => gotoTarget("/sales/returns/new")} className="flex h-6 w-6 items-center justify-center rounded text-text-muted hover:bg-blue-50 hover:text-blue-600" title="تعديل"><Pencil className="h-3.5 w-3.5" /></button>
  ...
  const itemColumns = [
    ...
    { id: "line_total", header: "الإجمالي", width: 105,
      headerClass: "text-center px-2 font-black uppercase tracking-widest text-text-muted",
      cellClass: "px-2 number-fmt text-sm text-success-text text-center",
    ...
    { id: "actions", header: "", width: 50, cellClass: "px-2",
      render: (r) => (
        <div className="flex gap-1 justify-center">
          <button onClick={(e) => { e.stopPropagation(); gotoTarget("/sales/returns/new"); }} className="flex h-6 w-6 items-center justify-center rounded text-text-muted hover:bg-blue-50 hover:text-blue-600" title="تعديل"><Pencil className="h-3.5 w-3.5" /></button>
        </div>
      )},
  ];
  ...
  return (
    <>
      <Modal open={open} onClose={onClose} title="سجل مرتجعات المبيعات" maxWidth="max-w-7xl" onDetach={handleDetach} showDetach={true} modalType="sales-return-today">
        <div className="flex flex-col gap-4">
          {/* Context banner */}
          ...
          <div className="flex items-center gap-0 rounded-sm bg-bg-overlay overflow-hidden border border-border-subtle">
            ...
            <div className="flex flex-col items-center px-4 py-3 shrink-0">
              ...
              <span className="font-mono text-[22px] font-black text-danger-text leading-none">{formatMoney(summary.total)}</span>
            </div>
          </div>
          {/* Table */}
