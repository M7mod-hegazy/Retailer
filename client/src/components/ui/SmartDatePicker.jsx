...
export default function SmartDatePicker({ dateFrom, dateTo, setDateFrom, setDateTo, theme = "amber", maxToday = true }) {
  ...
  return (
    <div className="flex flex-col sm:flex-row items-center gap-2 bg-bg-overlay/80 p-1.5 rounded-2xl border border-border-subtle">
      {/* Pills */}
      <div className="flex items-center gap-1 bg-border-normal/50 p-1 rounded-xl shrink-0">
        <button 
          onClick={() => handleModeChange("today")}
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "today" ? `bg-bg-surface ${activeCls} shadow-sm` : "text-text-secondary hover:text-text-primary"}`}
        >
          اليوم
        </button>
        <button 
          onClick={() => handleModeChange("single")}
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "single" ? `bg-bg-surface ${activeCls} shadow-sm` : "text-text-secondary hover:text-text-primary"}`}
        >
          يوم محدد
        </button>
        <button 
          onClick={() => handleModeChange("range")}
          className={`px-3 py-1.5 rounded-lg text-[11px] font-black transition-all ${mode === "range" ? `bg-bg-surface ${activeCls} shadow-sm` : "text-text-secondary hover:text-text-primary"}`}
        >
          فترة
        </button>
      </div>
      ...
      <AnimatePresence mode="wait">
        ...
        {mode === "range" && (
          <motion.div initial={{ width: 0, opacity: 0 }} animate={{ width: "auto", opacity: 1 }} exit={{ width: 0, opacity: 0 }} className="flex items-center gap-1 overflow-hidden">
            <span className="text-[11px] font-black text-text-muted shrink-0">من</span>
            <input 
            ...
            <span className="text-[11px] font-black text-text-muted shrink-0">إلى</span>
            <input 
