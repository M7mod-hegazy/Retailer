...
const STATUS_PREVIEW = {
  ...
  cancelled: { label: "ملغي",   cls: "bg-bg-overlay text-text-secondary" },
  ...
  return (
    <div className="flex flex-col gap-4">
      {loading ? (
        ...
        <>
          {/* Header info */}
          <div className="rounded-sm bg-bg-overlay border border-border-normal px-4 py-3 flex flex-wrap gap-x-5 gap-y-1.5 text-sm">
            <span className="font-black text-text-primary">فاتورة #{d.invoice_no}</span>
            <span className={`px-2 py-0.5 rounded text-[11px] font-black ${statusInfo.cls}`}>{statusInfo.label}</span>
            <span className="text-text-secondary">العميل: {d.customer_name ? (
              <strong>{d.customer_name}</strong>
            ) : (
              <>
                <strong className="text-emerald-700">🚶 عميل نقدي</strong>
                {d.walk_in_phone && (
                  <span className="text-text-secondary"> — {d.walk_in_name ? `${d.walk_in_name} ` : ""}<span className="font-mono" dir="ltr">{d.walk_in_phone}</span></span>
                )}
              </>
            )}</span>
            ...
            <span className="font-bold text-text-primary">طريقة الدفع: {PAYMENT_LABELS_PREVIEW[d.payment_type] || d.payment_type || "—"}</span>
          </div>
              ...
              <tbody>
                {(d.lines || []).map((l, i) => (
                  <tr key={i} className="border-b border-border-subtle hover:bg-bg-overlay">
                    ...
                    <td className="px-3 py-2.5 text-center number-fmt text-text-primary">{formatMoney(l.line_total || (l.quantity * l.unit_price))}</td>
                  </tr>
                ))}
              </tbody>
            </table>
              ...
              <div className="flex-1 min-w-[160px] rounded-sm border border-border-normal bg-bg-overlay px-4 py-3 space-y-1.5 text-2sm">
                ...
                {d.payments.map((p, i) => (
                  <div key={i} className="flex justify-between">
                    <span className="text-text-secondary">{p.method_name || PAYMENT_LABELS_PREVIEW[p.method] || p.method}</span>
