    ...
    >
      ...
      >
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
          ...
          <button
            onClick={onClose}
            className="p-1.5 rounded-lg hover:bg-gray-100 text-text-muted hover:text-text-primary transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          <div className="flex-1 flex items-center justify-center bg-black/40 p-4 relative min-h-[400px]">
            {allImages.length > 0 ? (
              ...
              <div className="text-center text-text-muted">
                ...
                <p className="text-sm font-bold">لا توجد صور</p>
              </div>
            )}
          </div>
          ...
          <div className="w-64 border-r border-gray-200 flex flex-col">
            <div className="px-4 py-3 border-b border-gray-200 flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button onClick={selectAll} className="text-xs font-bold text-text-link hover:underline">
                  الكل
                </button>
                <button onClick={selectNone} className="text-xs font-bold text-text-muted hover:underline">
                  إلغاء
                </button>
              </div>
            </div>
            <div className="flex-1 overflow-y-auto p-3 space-y-2">
              {allImages.length === 0 && (
                <p className="text-xs text-text-muted text-center py-8">لا توجد صور</p>
              )}
              {allImages.map((url, i) => (
                <button
                  key={i}
                ...
                >
                  ...
                  <span className="text-xs text-text-secondary flex-1 text-right truncate">
                    صورة {i + 1}
                  </span>
                  {selected.has(url) ? (
