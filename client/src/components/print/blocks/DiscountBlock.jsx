...
export default function DiscountBlock({ invoice = {}, settings: s, props = {}, family, editing }) {
  ...
  if (family === "page") {
    ...
    if (variant === "underline-accent") {
      return (
        <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderBottom: "2px solid #dc2626" }}>
          <span style={{ color: "#b91c1c", fontWeight: 700 }}>{label}</span>
          <span style={{ fontWeight: 800, color: "#b91c1c" }}>- {currency} {smartFormat(displayDiscount, s)}</span>
        </div>
      );
    ...
    if (variant === "inline") {
      return (
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: "9px", color: "#94a3b8", padding: "1px 0" }}>
          <span>{label}</span>
          <span style={{ fontWeight: 600, color: "#dc2626" }}>- {currency} {smartFormat(displayDiscount, s)}</span>
        </div>
      );
    }
  ...
  if (variant === "underline-accent") {
    return (
      <div style={{ display: "flex", justifyContent: "space-between", borderBottom: "2px solid #000", padding: "2px 0", margin: "2px 0" }}>
  ...
  if (variant === "inline") {
    return (
      <div style={{ display: "flex", justifyContent: "space-between", fontSize: "8px", opacity: 0.7 }}>
        ...
        <span style={{ fontWeight: 700 }}>- {currency} {smartFormat(displayDiscount, s)}</span>
      </div>
    );
  }
  ...
  if (variant === "centered") {
    return (
      <div style={{ textAlign: "center", fontSize: "9px", borderTop: "1px dashed #000", borderBottom: "1px dashed #000", padding: "2px 0", margin: "2px 0", fontWeight: 900 }}>
        {label ? `${label}: ` : ""}- {currency} {smartFormat(displayDiscount, s)}
      </div>
    );
  }
