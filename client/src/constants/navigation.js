import {
  ...
  Database, Warehouse, Scale,
  ShieldCheck, PieChart, Fingerprint, UsersRound, Settings, ArrowUpCircle, ClipboardCheck,
  ShoppingBag, Smartphone, Wrench, UtensilsCrossed, Gem, MessageSquare,
} from "lucide-react";
...
export const NAV_MODULES = [
  ...
  },
  {
    ...
    items: [
      ...
      { path: "/stock/serials", label: "بحث السيريال / IMEI", icon: Smartphone, pageKey: "serial_search", featureKey: "feature_serials" },
      { path: "/repairs", label: "أوامر الصيانة", icon: Wrench, pageKey: "repair_orders", featureKey: "feature_repair_orders" },
      { path: "/restaurant/tables", label: "طاولات المطعم", icon: UtensilsCrossed, pageKey: "restaurant_tables", featureKey: "feature_restaurant" },
      { path: "/restaurant/modifier-groups", label: "الإضافات والمقادير", icon: UtensilsCrossed, pageKey: "restaurant_modifiers", featureKey: "feature_restaurant" },
      { path: "/gold/rates", label: "أسعار الذهب", icon: Gem, pageKey: "gold_pricing", featureKey: "feature_gold" },
  ...
  },
  {
    ...
    items: [
      ...
      { path: "/sync", label: "المزامنة مع المتجر", icon: ArrowRightLeft, pageKey: "sync" },
    ],
  },
];
