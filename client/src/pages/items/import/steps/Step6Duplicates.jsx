...
export default function Step6Duplicates({ wizard }) {
  ...
  const [justAppliedBulk, setJustAppliedBulk] = useState(null);
  ...
  const decidedCount = duplicateGroups.filter((g) => {
    ...
    return wizard.duplicatePolicies[key] !== undefined;
  }).length;
  ...
  function handleBulkAction(policy) {
    wizard.applyDuplicatePolicyToAll(policy);
    setJustAppliedBulk(policy);
    setTimeout(() => setJustAppliedBulk(null), 2000);
  }
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        <div className="rounded-2xl border border-amber-200 bg-white p-5 shadow-sm flex flex-col justify-between">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <span className={`inline-flex items-center gap-1.5 rounded-lg px-3 py-1.5 text-xs font-black ring-1 ${
                duplicateGroups.length === 0
                  ...
                  : decidedCount === duplicateGroups.length && duplicateGroups.length > 0
                  ? "bg-emerald-50 text-emerald-700 ring-emerald-200"
                  : "bg-amber-50 text-amber-700 ring-amber-200"
              }`}>
                {duplicateGroups.length === 0 ? <CheckCircle2 className="h-3.5 w-3.5" /> : <AlertTriangle className="h-3.5 w-3.5" />}
                {decidedCount} / {duplicateGroups.length} مقرر
              </span>
            </div>
          ...
          <div className="mt-4 inline-flex items-center gap-2 rounded-xl bg-slate-100 px-4 py-2 text-xs font-black text-slate-700 ring-1 ring-slate-200">
            ...
            القرار العام: {wizard.duplicateMode === "warehouse" ? "توزيع على المخازن" : "دمج الكميات"}
          </div>
        </div>
        ...
        <div className="grid gap-3 rounded-2xl border border-slate-200 bg-slate-50/60 p-4.5 shadow-inner">
          <button
            ...
            className={`w-full rounded-xl py-3.5 text-sm font-black transition-all duration-200 inline-flex items-center justify-center gap-2 ${wizard.duplicateMode === "combine" && !justAppliedBulk ? "bg-primary text-white shadow-md shadow-slate-900/10" : justAppliedBulk === "combine" ? "bg-emerald-700 text-white" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 active:scale-[0.98]"}`}
          >
            {justAppliedBulk === "combine" ? <CheckCircle2 className="h-4 w-4" /> : null}
            دمج كل التكرارات
          </button>
          ...
          <button
            ...
            className={`w-full rounded-xl py-3.5 text-sm font-black transition-all duration-200 inline-flex items-center justify-center gap-2 ${wizard.duplicateMode === "warehouse" && !justAppliedBulk ? "bg-primary text-white shadow-md shadow-slate-900/10" : justAppliedBulk === "warehouse" ? "bg-indigo-600 text-white" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 active:scale-[0.98]"}`}
          >
            {justAppliedBulk === "warehouse" ? <CheckCircle2 className="h-4 w-4" /> : null}
      ...
      {duplicateGroups.length > 0 && (
        <div className="space-y-3">
          ...
          {displayedGroups.map((group) => {
            ...
            return (
              <div key={row.__rowNumber} className={`rounded-2xl border p-5 shadow-sm transition-all duration-200 ${isOverridden ? (policy === "warehouse" ? "border-indigo-200 bg-indigo-50/30 ring-1 ring-indigo-100" : "border-emerald-200 bg-emerald-50/30 ring-1 ring-emerald-100") : "border-slate-200 bg-white hover:shadow-md"}`}>
                <div className="grid gap-5 xl:grid-cols-[1fr_280px]">
                  <div>
                    <div className="flex flex-wrap items-center gap-3">
                      <span className="text-lg font-black text-slate-900 font-display">{row.name || row.code}</span>
                      <span className={`inline-flex items-center gap-1 rounded-lg px-2.5 py-1 text-xs font-black ring-1 ${policy === "warehouse" ? "bg-indigo-50 text-indigo-700 ring-indigo-200" : "bg-slate-100 text-slate-600 ring-slate-200"}`}>
                        {isOverridden ? <CheckCircle2 className="h-3 w-3" /> : null}
                  ...
                  <div className="flex flex-col justify-center gap-3">
                    ...
                    <button
                      ...
                      className={`w-full rounded-xl py-3 text-xs font-black transition-all duration-200 inline-flex items-center justify-center gap-2 ${policy === "combine" && isOverridden ? "bg-emerald-700 text-white shadow-md ring-2 ring-emerald-300" : policy === "combine" && !isOverridden ? "bg-primary text-white shadow-md shadow-slate-900/10" : "border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 active:scale-[0.98]"}`}
                    >
                      {isOverridden && policy === "combine" ? <CheckCircle2 className="h-3.5 w-3.5" /> : null}
                      دمج هذا الصنف
                    </button>
                    <button
                      type="button"
                      onClick={() => wizard.setDuplicatePolicyForGroup(row, "warehouse")}
                      className={`w-full rounded-xl py-3 text-xs font-black transition-all duration-200 inline-flex items-center justify-center gap-2 ${policy === "warehouse" && isOverridden ? "bg-indigo-600 text-white shadow-md ring-2 ring-indigo-400" : policy === "warehouse" && !isOverridden ? "bg-indigo-600 text-white shadow-md shadow-indigo-600/10 ring-2 ring-indigo-400" : "border border-slate-200 bg-white text-slate-700 hover:bg-indigo-50 hover:border-indigo-200 active:scale-[0.98]"}`}
                    >
                      {isOverridden && policy === "warehouse" ? <CheckCircle2 className="h-3.5 w-3.5" /> : null}
                      توزيع هذا الصنف
                    </button>
                    {isOverridden && (
                      <button
