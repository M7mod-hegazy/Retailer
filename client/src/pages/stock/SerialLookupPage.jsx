...
function SerialCard({ data }) {
  const status = STATUS_LABELS[data.status] || { label: data.status, icon: Clock, color: "text-text-secondary bg-bg-overlay" };
  const Icon = status.icon;
  const warrantyOk = data.warranty_expires && new Date(data.warranty_expires) >= new Date();
  return (
    <div className="rounded-xl border border-border-normal bg-bg-surface shadow-sm p-6 space-y-4 max-w-xl">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h3 className="text-lg font-black text-text-primary">{data.serial}</h3>
          <p className="text-sm font-bold text-text-secondary mt-0.5">{data.item_name}</p>
          {data.item_barcode && <p className="text-xs font-mono text-text-muted mt-0.5">{data.item_barcode}</p>}
        </div>
      ...
      <div className="grid grid-cols-2 gap-3 text-sm">
        {data.sold_at && (
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">تاريخ البيع</span>
            <p className="font-bold text-text-primary">{data.sold_at.slice(0, 10)}</p>
          </div>
        )}
        {data.warranty_months && (
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">الضمان</span>
        ...
        {data.invoice_id && (
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">رقم الفاتورة</span>
            <p className="font-bold text-text-primary">#{data.invoice_id}</p>
          </div>
        )}
        {data.returned_at && (
          <div>
            <span className="text-[10px] font-black uppercase tracking-widest text-text-muted">تاريخ الإعادة</span>
      ...
      {data.notes && (
        <p className="text-xs font-bold text-text-secondary bg-bg-overlay rounded-lg p-3">{data.notes}</p>
      )}
    </div>
  );
}
  ...
  return (
    <div className="standard-page-container space-y-6 pb-20">
      <div>
        ...
        <h1 className="text-2xl font-black text-text-primary">البحث عن سيريال / IMEI</h1>
        <p className="text-sm font-bold text-text-muted mt-1">اعرف حالة أي جهاز: هل هو في المخزون، مُباع، أو مُعاد؟</p>
      </div>
      ...
      <form onSubmit={handleSearch} className="flex gap-3 max-w-lg">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border-normal bg-bg-surface py-3 pl-4 pr-10 text-sm font-bold text-text-primary placeholder:text-text-muted focus:border-slate-800 focus:outline-none shadow-sm"
          />
        </div>
