...
function statusColor(s) {
  ...
  return "bg-bg-overlay text-text-secondary border-border-normal";
}
      ...
      <div className="px-3 py-2.5 bg-amber-100/50 border-b border-amber-200/60 flex items-center justify-between gap-2 relative z-10">
        ...
        <div className="flex items-center gap-1.5 shrink-0">
          {invoice.payment_type && (
            <span className="text-[11px] text-amber-700 font-bold bg-bg-surface px-1.5 py-0.5 rounded-md border border-amber-200">{paymentTypeLabel(invoice.payment_type)}</span>
          )}
          {invoice.status && (
...
export default function SalesReturnFormPage() {
  ...
  async function handleSave(opts = {}) {
    ...
    try {
      ...
      else if (printPreview || waSendOpen) {
      ...
      <div dir="rtl" className="flex h-full flex-col bg-bg-overlay overflow-hidden relative">
        ...
        <div className="flex flex-1 flex-col items-center justify-center gap-12 px-4 relative z-10 pb-16">
          <div className="flex flex-col items-center gap-4 text-center">
            ...
            <h1 className="text-[32px] font-black text-text-primary tracking-tight">إنشاء مرتجع مبيعات</h1>
          ...
          <div className="flex flex-col md:flex-row gap-6 w-full max-w-3xl">
            <button onClick={() => selectMode("direct")}
              className="group relative flex-1 flex flex-col justify-between rounded-[2.5rem] bg-bg-surface border border-border-normal/60 p-8 overflow-hidden transition-all duration-300 hover:-translate-y-2 shadow-sm hover:shadow-2xl hover:border-emerald-300 text-right">
              ...
              <div className="bg-bg-overlay w-16 h-16 rounded-[1.5rem] flex items-center justify-center group-hover:bg-emerald-100 group-hover:scale-110 transition-all duration-500 mb-8 border border-border-subtle">
                <RotateCcw className="h-8 w-8 text-text-muted group-hover:text-emerald-700 transition-colors" />
              </div>
              <div className="relative z-10 flex flex-col">
                <span className="text-[22px] font-black text-text-primary mb-2">مرتجع مباشر (حر)</span>
                <span className="text-sm font-bold text-text-secondary leading-relaxed">إضافة الأصناف يدوياً وتحديد الكميات والأسعار بدون الارتباط بفاتورة مبيعات مسبقة.</span>
              </div>
            </button>
  ...
  // ══ ACTIVE SCREEN ══
  return (
    <div dir="rtl" className="flex h-full flex-col bg-bg-overlay overflow-hidden animate-fade-in relative">
          ...
          <>
            ...
            {isEditMode && (
              <DocumentActionButton
                ...
                className="bg-[#25D366]/10 text-[#25D366] hover:bg-[#25D366]/20 border border-[#25D366]/20 flex items-center gap-1.5"
                icon={WhatsAppIcon}
              >
                واتساب
              </DocumentActionButton>
          ...
          <div className="flex flex-col gap-5 p-5">
            ...
            <div className="flex flex-col gap-1.5" data-help="sr-form-customer">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-bold text-text-secondary uppercase tracking-widest">العميل</label>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full h-10 rounded-xl border px-3 text-sm font-bold outline-none disabled:cursor-not-allowed disabled:opacity-60 transition-all shadow-sm placeholder:font-normal placeholder:text-text-muted ${hasCustomerBalance ? "border-amber-300 bg-amber-50 text-amber-900 focus:border-amber-400 focus:ring-2 focus:ring-amber-100" : "border-border-normal bg-bg-overlay text-text-primary focus:border-emerald-400 focus:bg-bg-surface focus:ring-2 focus:ring-emerald-100"}`}
                />
              ...
              {customerLockedFromInvoice && !isLocked && <p className="text-[11px] text-text-muted font-medium">العميل محدد من الفاتورة الأصلية</p>}
              {customer?.id && (
              ...
              {customer?.id && customerBalance !== null && (
                <div className="flex items-center gap-1.5 flex-wrap mt-0.5">
                  <div className={`text-[11px] font-black px-2 py-1 rounded-sm border ${customerBalance > 0 ? "text-amber-700 bg-amber-100/50 border-amber-200" : "text-text-secondary bg-bg-overlay/50 border-border-normal"}`}>
            ...
            {customer && customerBalance !== null && (
              <div className="rounded-xl border border-border-normal bg-bg-surface p-4 shadow-sm flex flex-col gap-3 relative overflow-hidden">
                ...
                {ajalDebt > 0 && (
                  <div className="flex justify-between items-center text-[11px] pt-2 mt-1 border-t border-border-subtle border-dashed">
                ...
                <Link to={`/definitions/customers/${customer.id}`} className="flex items-center justify-center gap-1 mt-2 py-1.5 rounded-lg bg-bg-overlay text-[11px] font-bold text-text-secondary hover:text-emerald-700 hover:bg-emerald-50 transition-colors">
                ...
                <div className="flex flex-col gap-2 rounded-xl border border-amber-200 bg-amber-50/60 px-4 py-3 shadow-sm">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[11px] font-bold text-text-secondary">خصم الفاتورة الكامل</label>
                    ...
                    <div className="flex flex-col gap-0.5">
                      <label className="text-[11px] font-bold text-text-secondary">زيادة الفاتورة الكاملة</label>
                    ...
                    <div className="flex items-center gap-1">
                      {canEditTaxRate ? (
                        <input
                          ...
                          className="w-16 rounded-lg border border-indigo-200 bg-bg-surface px-2 py-1 text-center text-sm number-fmt-primary text-indigo-700 outline-none focus:border-indigo-400 focus:ring-1 focus:ring-indigo-100 disabled:opacity-60"
                        />
                      ) : (
            ...
            <div className="flex flex-col gap-1.5" data-help="sr-form-refund">
              <label className="text-[11px] font-bold text-text-secondary uppercase tracking-widest">طريقة الاسترداد</label>
                ...
                {[
                  ...
                  return (
                    <button key={opt.value} onClick={() => !disabled && setRefundMethod(opt.value)} disabled={disabled}
                      ...
                      className={`flex-1 rounded-lg py-2 px-1 text-center transition-all disabled:cursor-not-allowed ${active ? "bg-bg-surface text-emerald-700 shadow-sm ring-1 ring-slate-200/50" : noCustomerBlocked ? "bg-red-50 text-red-500 border border-dashed border-red-200" : "text-text-secondary hover:text-text-primary disabled:opacity-40"}`}>
                ...
                <div className="mt-3 flex flex-col gap-2">
                  <div className="relative">
                    <select value={reason} onChange={e => setReason(e.target.value)} disabled={isLocked}
                      className="w-full h-10 rounded-xl border border-border-normal bg-bg-overlay px-3 text-sm font-bold text-text-primary outline-none focus:border-emerald-400 focus:bg-bg-surface focus:ring-2 focus:ring-emerald-100 disabled:cursor-not-allowed disabled:opacity-60 transition-all shadow-sm appearance-none">
                  ...
                  {reason === "other" && !isLocked && (
                    <input value={reasonOther} onChange={e => setReasonOther(e.target.value)} placeholder="اذكر السبب بتفصيل..."
                      className="w-full h-10 rounded-xl border border-border-normal bg-bg-surface px-3 text-2sm font-medium text-text-primary outline-none focus:border-emerald-400 focus:ring-2 focus:ring-emerald-100 shadow-sm transition-all" />
                  )}
                </div>
              )}
            </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              {isLocked ? (
                ...
                <textarea
                  ...
                  className="w-full resize-none rounded-xl border border-border-normal bg-bg-surface px-3 py-2 text-sm font-medium text-text-primary outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-100 transition-all"
                />
              )}
            </div>
                          ...
                          {ALL_COLUMNS_DIRECT.filter(c => c.id !== "actions").map(c => (
                            <label key={c.id} className="flex items-center gap-2 px-2 py-1.5 text-xs font-medium text-text-primary hover:bg-bg-overlay rounded cursor-pointer">
                              <input type="checkbox" checked={visibleColumns.includes(c.id)}
                                ...
                                className="h-3.5 w-3.5 rounded border-border-strong accent-emerald-600" />
                              {c.label}
                            </label>
                          ))}
                        </div>
                          ...
                          {visibleColumns.includes("warehouse") && (function(){
                            const whEl = !isLocked ? (
                              <div className="flex flex-col items-center gap-1">
                                <select value={l.warehouse_id} data-grid-cell data-row={idx} data-col="warehouse_id" onChange={e => updateCartWarehouse(l.key, e.target.value)} className="h-7 w-full rounded border border-border-normal bg-bg-overlay px-1.5 text-2sm font-bold text-text-primary outline-none focus:border-emerald-400 focus:bg-bg-surface focus:ring-1 focus:ring-emerald-100 transition-colors cursor-pointer">
                            ...
                            {!isLocked ? (() => {
                              ...
                              const healthClasses = returnHealth.level === "loss"
                                ...
                                : returnHealth.level === "thin"
                                  ...
                                  : "border-border-normal bg-bg-overlay text-text-primary focus:border-emerald-400 focus:bg-bg-surface focus:ring-emerald-200";
                              ...
                              <span className="text-sm font-black text-text-primary number-fmt">{formatMoney(l.unit_price)}</span>
                            )}
                          </td>}
                  ...
                  <div className="flex flex-1 flex-col gap-4 overflow-hidden">
                    <div className="flex items-center justify-between rounded-lg border border-emerald-200 bg-emerald-50 px-4 py-3 shrink-0">
                      <div className="flex items-center gap-4 text-sm">
                        ...
                        {loadedInvoice.customer_name && <span className="text-text-secondary">العميل: <strong>{loadedInvoice.customer_name}</strong></span>}
                        <span className="text-text-secondary">{formatDate(loadedInvoice.created_at)}</span>
                    ...
                    <div className="flex-1 overflow-x-auto overflow-y-auto rounded-lg border border-border-normal bg-bg-surface shadow-sm" data-help="sr-form-items">
                      <table className="w-full text-right">
                        <thead className="border-b border-border-normal bg-bg-overlay sticky top-0">
                          <tr>
                            ...
                            <th className="px-3 py-3 text-[11px] font-bold text-text-secondary text-center">السعر</th>
                            <th className="px-3 py-3 text-[11px] font-bold text-text-secondary text-center">الكمية الأصلية</th>
                            <th className="px-3 py-3 text-[11px] font-bold text-text-secondary text-center">إجمالي الفاتورة</th>
                        ...
                        <tbody>
                          {invoiceLines.map((l, idx) => {
                            ...
                            return (
                              <tr key={l.invoice_line_id} className={`border-b border-border-subtle transition-colors animate-slide-up ${l.checked ? "bg-emerald-50/50" : "hover:bg-bg-overlay"}`} style={{ animationDelay: `${idx * 50}ms` }}>
                                <td className="px-3 py-3 text-center">
                                  <input type="checkbox" checked={l.checked} onChange={() => !isLocked && toggleInvoiceLine(l.invoice_line_id)} disabled={isLocked}
                                    className="h-4 w-4 rounded border-border-strong accent-emerald-600 cursor-pointer disabled:cursor-not-allowed" />
                                </td>
                                ...
                                <td className="px-3 py-3 text-center">
                                  <div className="flex items-center justify-center gap-1">
                                    <span className={`text-2sm number-fmt ${l.purchase_price > 0 && l.unit_price < l.purchase_price ? "text-rose-600 font-bold" : "text-text-secondary"}`}>
                                ...
                                <td className="px-3 py-3 text-center">
                                  <input type="number" min="0" max={l.original_qty - l.already_returned} value={l.qty_to_return} data-help="sr-form-qty"
                                    ...
                                    className="w-16 rounded-sm border border-border-normal px-2 py-1 text-center text-sm font-black text-text-primary outline-none focus:border-emerald-400 disabled:bg-bg-overlay disabled:text-text-muted disabled:cursor-not-allowed" />
                                </td>
      ...
      <Modal open={showSaveConfirmModal} onClose={() => !isSaving && setShowSaveConfirmModal(false)} title="تأكيد حفظ المرتجع" showDetach={false}>
        <div className="flex flex-col gap-6 mt-2 animate-modal-enter">
          <div className="flex items-start gap-4 p-4 rounded-2xl bg-emerald-50/50 border border-emerald-100/80">
            ...
            <div>
              <p className="text-sm font-black text-text-primary">تأكيد حفظ المرتجع</p>
          ...
          {isSaving ? (
            <div className="flex flex-col items-center justify-center py-8 gap-3">
              ...
              <p className="text-2sm font-black text-text-secondary animate-pulse">جاري حفظ وتجهيز المرتجع...</p>
            </div>
          ) : (
            <div className="grid grid-cols-3 gap-3">
              ...
              <button
                type="button"
                onClick={() => { setShowSaveConfirmModal(false); setPrintPreview(true); }}
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <span className="text-[9px] font-bold text-text-muted leading-tight">طباعة إيصال الاسترداد فوراً</span>
                </div>
              </button>
              ...
              <button
                type="button"
                onClick={() => { setShowSaveConfirmModal(false); setWaSendOpen(true); }}
              ...
              >
                ...
                <div className="flex flex-col gap-0.5 min-w-0">
                  ...
                  <span className="text-[9px] font-bold text-text-muted leading-tight">إرسال الإيصال عبر واتساب</span>
                </div>
              </button>
      ...
      <Modal open={showWarningModal} onClose={() => setShowWarningModal(false)} title="تأكيد الإلغاء" showDetach={false}>
        <div className="flex flex-col gap-5 animate-modal-enter">
          ...
          <div className="flex gap-3 justify-end">
            <button onClick={() => setShowWarningModal(false)} className="rounded-md border border-border-normal px-5 py-2 text-sm font-bold text-text-secondary hover:bg-bg-overlay transition-all active:scale-[0.98]">إلغاء</button>
      ...
      <Modal open={showSwitchInvoiceWarning} onClose={() => setShowSwitchInvoiceWarning(false)} title="تغيير الفاتورة" showDetach={false}>
        <div className="flex flex-col gap-5 animate-modal-enter">
          <p className="text-sm text-text-primary">يوجد مرتجع قيد التحرير. هل تريد حفظه أولاً قبل اختيار فاتورة أخرى؟</p>
          <div className="flex gap-3 justify-end">
            <button onClick={() => setShowSwitchInvoiceWarning(false)} className="rounded-md border border-border-normal px-5 py-2 text-sm font-bold text-text-secondary hover:bg-bg-overlay transition-all active:scale-[0.98]">إلغاء</button>
      ...
      {waSendOpen && (
        <WhatsAppSendModal
          ...
          onBeforeSend={lastSavedReturn ? undefined : () => handleSave()}
          kind="return_receipt"
          invoice={lastSavedReturn ? {
