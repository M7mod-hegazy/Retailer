      ...
      <div dir="rtl" className="space-y-4 p-4">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-black text-text-secondary">اسم المجموعة</label>
            <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" required value={name} onChange={(e) => setName(e.target.value)} placeholder="مثل: حجم المشروب" />
          </div>
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1">
              <label className="text-xs font-black text-text-secondary">نوع الاختيار</label>
              <select className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" value={selectionType} onChange={(e) => setSelectionType(e.target.value)}>
                <option value="single">اختيار واحد</option>
                <option value="multi">اختيار متعدد</option>
              </select>
            </div>
            <div className="space-y-1">
              <label className="text-xs font-black text-text-secondary">الترتيب</label>
              <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" type="number" value={sortOrder} onChange={(e) => setSortOrder(e.target.value)} />
            </div>
          </div>
          ...
          <div className="space-y-1">
            <label className="text-xs font-black text-text-secondary">الإضافات في هذه المجموعة</label>
            <div className="max-h-48 overflow-y-auto rounded-lg border border-border-normal p-2 space-y-1">
              {allModifiers.length === 0 && <p className="text-xs text-text-muted">لا توجد إضافات — أضف إضافات أولاً</p>}
              {allModifiers.map((m) => (
                <label key={m.id} className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-bg-overlay cursor-pointer text-sm">
                  <input type="checkbox" checked={selectedModifierIds.includes(m.id)} onChange={() => toggleModifier(m.id)} className="rounded border-border-strong" />
                  <span className="font-bold text-text-primary">{m.name}</span>
                  {m.price_adjustment > 0 && <span className="text-xs text-text-muted">+{m.price_adjustment}₪</span>}
                </label>
              ))}
            </div>
          </div>
  ...
  return (
    <Modal open title={null} onClose={onClose} maxWidth="max-w-sm">
      <div dir="rtl" className="space-y-4 p-4">
        ...
        <form onSubmit={handleSave} className="space-y-4">
          <div className="space-y-1">
            <label className="text-xs font-black text-text-secondary">الاسم (عربي)</label>
            <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" required value={name} onChange={(e) => setName(e.target.value)} placeholder="مثل: إضافي جبنة" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-black text-text-secondary">الاسم (إنجليزي) — اختياري</label>
            <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" value={nameEn} onChange={(e) => setNameEn(e.target.value)} placeholder="Extra Cheese" />
          </div>
          <div className="space-y-1">
            <label className="text-xs font-black text-text-secondary">تعديل السعر (₪)</label>
            <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" type="number" step="0.01" value={priceAdjustment} onChange={(e) => setPriceAdjustment(e.target.value)} placeholder="0.00" />
          </div>
          <div className="flex justify-end gap-2 pt-2 border-t border-border-subtle">
            <Button type="button" variant="ghost" size="sm" onClick={onClose}>إلغاء</Button>
            <Button type="submit" size="sm" disabled={saving}>
...
function ModifierItemsTab() {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <label className="text-xs font-black text-text-secondary">اختر صنفاً لإدارة مجموعات إضافاته</label>
        <input className="w-full rounded-lg border border-border-normal px-3 py-2 text-sm" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="ابحث عن صنف..." />
      </div>
      {search && (
        <div className="max-h-60 overflow-y-auto rounded-xl border border-border-normal bg-bg-surface">
          {items.slice(0, 30).map((item) => (
            <button key={item.id} type="button" onClick={() => { setSelectedItemId(item.id); setSearch(item.name); }}
              className={`w-full text-right px-4 py-2.5 text-sm font-bold hover:bg-bg-overlay border-b border-border-subtle last:border-b-0 ${selectedItemId === item.id ? "bg-sky-50 text-sky-700" : "text-text-primary"}`}>
              {item.name}
              <span className="mr-2 text-xs text-text-muted font-mono">{item.code || item.item_code || ""}</span>
            </button>
          ))}
        </div>
      )}
      {selectedItemId && (
        <div className="rounded-xl border border-border-normal p-4 space-y-3">
          <h3 className="text-sm font-black text-text-primary">مجموعات الإضافات المرتبطة</h3>
          {(allGroups || []).length === 0 ? (
            <p className="text-xs text-text-muted">لا توجد مجموعات إضافات — أنشئ مجموعة أولاً</p>
          ) : (
            <div className="space-y-2">
              {(allGroups || []).map((g) => {
                const attached = attachedGroupIds.includes(g.id);
                return (
                  <label key={g.id} className={`flex items-center justify-between rounded-lg border px-4 py-2.5 cursor-pointer transition-all ${attached ? "border-emerald-200 bg-emerald-50/50" : "border-border-normal hover:border-border-strong"}`}>
                    <div className="flex items-center gap-2">
                      <input type="checkbox" checked={attached} onChange={() => toggleGroup(g.id, attached)} className="rounded border-border-strong" />
                      <span className="font-bold text-sm text-text-primary">{g.name}</span>
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${g.selection_type === "multi" ? "bg-blue-100 text-blue-700" : "bg-bg-overlay text-text-secondary"}`}>
                        {g.selection_type === "multi" ? "متعدد" : "واحد"}
                      </span>
                    </div>
                    {g.modifiers?.length > 0 && (
                      <span className="text-xs text-text-muted">{g.modifiers.map((m) => m.name).join("، ")}</span>
                    )}
                  </label>
                );
              })}
      ...
      {!selectedItemId && !search && (
        <p className="text-text-muted py-8 text-center">ابحث عن صنف لربط مجموعات الإضافات به</p>
      )}
    </div>
  );
}
  ...
  return (
    <FeatureRoute featureKey="feature_restaurant">
      <div className="space-y-6">
        ...
        {/* Tabs */}
        <div className="flex gap-1 border-b border-border-normal">
          {[
            ...
            <button key={t.id} onClick={() => setTab(t.id)}
              className={`px-4 py-3 text-sm font-black transition-all border-b-2 ${
                tab === t.id ? "border-slate-800 text-text-primary" : "border-transparent text-text-muted hover:text-text-secondary"
              }`}>
              {t.label}
            </button>
          ))}
        ...
        {tab === "groups" && (
          <div className="space-y-3">
            ...
            {loadingGroups ? (
              ...
              <p className="text-text-muted py-8 text-center">لا توجد مجموعات إضافات — أضف أول مجموعة</p>
            ) : (
              <div className="space-y-2">
                {(groups || []).map((g) => (
                  <div key={g.id} className="rounded-xl border border-border-normal bg-bg-surface overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-3 bg-bg-overlay/50">
                      <div className="flex items-center gap-3">
                        <span className="font-black text-text-primary">{g.name}</span>
                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${g.selection_type === "multi" ? "bg-blue-100 text-blue-700" : "bg-bg-overlay text-text-secondary"}`}>
                          {g.selection_type === "multi" ? "متعدد" : "واحد"}
                        </span>
                      ...
                      <div className="flex items-center gap-1">
                        <button type="button" onClick={() => setGroupModal(g)} className="p-1.5 rounded-lg hover:bg-border-normal text-text-muted hover:text-text-secondary"><Pencil className="h-4 w-4" /></button>
                        <button type="button" onClick={() => setGroupToDelete(g)} className="p-1.5 rounded-lg hover:bg-red-100 text-text-muted hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                      </div>
                    </div>
                    {g.modifiers?.length > 0 && (
        ...
        {tab === "modifiers" && (
          <div className="space-y-3">
            ...
            {loadingModifiers ? (
              ...
              <p className="text-text-muted py-8 text-center">لا توجد إضافات — أضف أول إضافة</p>
            ) : (
              <div className="overflow-x-auto rounded-xl border border-border-normal">
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-bg-overlay text-xs font-black text-text-secondary uppercase tracking-wider">
                  ...
                  <tbody>
                    {(modifiers || []).map((m) => (
                      <tr key={m.id} className="border-t border-border-subtle hover:bg-bg-overlay/50">
                        <td className="px-4 py-2.5 font-bold text-text-primary">{m.name}</td>
                        <td className="px-4 py-2.5 text-text-muted text-xs">{m.name_en || "—"}</td>
                        <td className="px-4 py-2.5 font-mono text-sm">{m.price_adjustment > 0 ? `+${m.price_adjustment}₪` : "—"}</td>
                        <td className="px-4 py-2.5 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <button type="button" onClick={() => setModifierModal(m)} className="p-1.5 rounded-lg hover:bg-border-normal text-text-muted hover:text-text-secondary"><Pencil className="h-4 w-4" /></button>
                            <button type="button" onClick={() => setModifierToDisable(m)} className="p-1.5 rounded-lg hover:bg-red-100 text-text-muted hover:text-red-600"><Trash2 className="h-4 w-4" /></button>
                          </div>
                        </td>
                      </tr>
                    ))}
