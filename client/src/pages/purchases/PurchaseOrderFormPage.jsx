...
export default function PurchaseOrderFormPage() {
  usePageTour('purchase_order_form');
  ...
  return (
    <div className="flex h-full min-h-[600px] flex-col bg-bg-overlay font-sans overflow-y-scroll px-4 lg:px-8 pb-6">
      {/* Header */}
      ...
      <main className="flex min-h-0 flex-1 flex-col p-4" style={{ paddingBottom: panelEffectiveCollapsed ? "calc(1rem + var(--bottom-bar-h, 90px))" : "1rem" }}>
        ...
        <div className="flex gap-4 flex-1 min-h-0">
          ...
          <div className="flex flex-1 flex-col gap-3 min-w-0">
            ...
            <section className="grid grid-cols-3 gap-4 rounded-md border border-border-strong bg-bg-surface p-4 shadow-sm">
                <div className="relative flex flex-col gap-1" data-help="po-form-supplier">
                  <label className="text-[11px] font-bold text-text-secondary">المورد المقترح</label>
                  <div className="relative">
                    <User className="absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-text-muted" />
                    <input
                      ...
                      className="w-full border border-border-strong rounded-sm py-2 pl-3 pr-9 text-2sm font-bold text-text-primary outline-none focus:border-slate-800"
                ...
                <div className="flex flex-col gap-1 col-span-2" data-help="po-form-notes">
                  <label className="text-[11px] font-bold text-text-secondary">ملاحظات الطلب</label>
                  <div className="relative">
                    <FileText className="absolute right-3 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-text-muted" />
                    <input 
                      ref={notesRef}
                      type="text" 
                      value={notes} 
                      ...
                      className="w-full border border-border-strong rounded-sm bg-bg-surface py-2 pl-3 pr-9 text-2sm font-bold text-text-primary outline-none focus:border-slate-800"
            ...
            {/* Grid */}
            <section className="flex flex-1 flex-col overflow-hidden rounded-md border border-border-strong bg-bg-surface min-h-[500px]" data-help="po-form-items-table">
              <div className="flex items-center justify-between px-3 py-2 shrink-0 border-b border-border-subtle">
                <div className="flex items-center gap-1"><div className="text-2sm font-bold text-text-secondary">الأصناف ({lines.length})</div><ShortcutKbd id="grid.editLast" /></div>
              ...
              <DataGrid
                ...
                className="border-0"
                containerClass="flex-1 overflow-x-auto overflow-y-auto rounded-xl border border-border-normal bg-bg-surface shadow-sm scrollbar-thin scrollbar-thumb-slate-300 scrollbar-track-transparent min-h-0"
                columns={[
                  {
                    id: "index", header: "#", width: 40, sortable: false, headerClass: "text-center", cellClass: "text-center font-mono text-2sm text-text-muted border-l border-border-subtle",
                    render: (_, i) => i + 1
                  },
                  {
                    id: "code", header: "الكود", width: 100, sortable: true, headerClass: "text-center", cellClass: "font-mono text-2sm font-black tracking-wider text-text-secondary border-l border-border-subtle text-center",
                    render: (l) => l.barcode || l.code || l.item_code || '-'
                  },
                  {
                    id: "name", header: "عنوان الصنف والباركود", width: 220, sortable: true, cellClass: "font-bold text-text-primary border-l border-border-subtle px-2", headerClass: "text-right px-2",
                    render: (l) => {
                  ...
                  {
                    id: "quantity", header: "الكمية", width: 80, sortable: true, headerClass: "text-center", cellClass: "text-center number-fmt-primary text-sm text-text-primary border-l border-border-subtle",
                    render: (l) => Number(l.quantity)
                  },
                  {
                    id: "unit_id", header: "الوحدة", width: 80, sortable: false, headerClass: "text-center", cellClass: "text-center text-2sm font-bold text-text-secondary border-l border-border-subtle",
                    render: (l) => (units.find(u => String(u.id) === String(l.unit_id))?.name || "أساسية")
                  },
                  {
                    id: "unit_cost", header: "التكلفة", width: 90, sortable: true, headerClass: "text-center", cellClass: "text-center number-fmt-primary text-sm text-text-secondary border-l border-border-subtle",
                    render: (l) => formatNumber(l.unit_cost)
                  },
                  {
                    id: "selling_price", header: "سعر البيع", width: 90, sortable: true, headerClass: "text-center", cellClass: "text-center number-fmt-primary text-sm text-emerald-700 border-l border-border-subtle",
                    render: (l) => formatNumber(l.selling_price || 0)
                  },
                  {
                    id: "wholesale_price", header: "سعر الجملة", width: 90, sortable: true, headerClass: "text-center", cellClass: "text-center number-fmt-primary text-sm text-text-secondary border-l border-border-subtle",
                    render: (l) => formatNumber(l.wholesale_price || 0)
                  },
                  {
                    id: "profit", header: "الربح", width: 100, sortable: true, headerClass: "text-center", cellClass: "text-center border-l border-border-subtle",
                  ...
                  },
                  {
                    ...
                    render: (l, i) => (
                      <select value={l.warehouse_id || ""} data-grid-cell data-row={i} data-col="warehouse_id" onChange={(e) => updateLineField(i, "warehouse_id", e.target.value)}
                        className="w-full h-[40px] text-[11px] font-bold outline-none border-0 ring-0 text-center truncate cursor-pointer bg-transparent text-text-primary focus:bg-indigo-50">
                        {warehouses.map(w => {
                  ...
                  {
                    id: "total_cost", header: "إجمالي المتوقع", width: 140, sortable: true, headerClass: "text-left px-2", cellClass: "text-left px-2 number-fmt-primary text-sm text-text-primary bg-bg-overlay/50 border-l-0",
                    sortValue: (l) => l.total,
                  ...
                  },
                  {
                    ...
                    render: (_, i) => (
                      <button onClick={() => setLines(prev => prev.filter((_, idx) => idx !== i))} className="inline-flex h-[40px] w-full items-center justify-center text-text-muted hover:bg-bg-overlay hover:text-rose-600 transition-colors focus:outline-none">
              ...
              />
               <div className="flex shrink-0 items-center justify-between border-t border-border-normal bg-bg-overlay px-6 py-3 rounded-b-xl">
                  <div className="flex items-center gap-6">
                     <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-text-muted">إجمالي الأصناف:</span>
                        <span className="text-sm font-black text-text-primary">{totals.items}</span>
                     </div>
                     <div className="flex items-center gap-2">
                        <span className="text-[11px] font-bold text-text-muted">إجمالي الكميات:</span>
                        <span className="text-sm font-black text-text-primary number-fmt">{lines.reduce((acc, l) => acc + l.quantity, 0)}</span>
                     </div>
                     {(discount > 0 || increase > 0) && (
                       <div className="flex items-center gap-2">
                         <span className="text-[11px] font-bold text-text-muted">الإجمالي الفرعي:</span>
                         <span className="text-sm font-black text-text-secondary number-fmt">{formatNumber(totals.sub)}</span>
                       </div>
                     )}
                  </div>
                  <div className="flex items-center gap-2">
                     ...
                     <span className="text-2sm font-bold text-text-secondary uppercase tracking-wider">القيمة الإجمالية المتوقعة</span>
                     <span className="text-[20px] font-black text-text-primary number-fmt">{formatNumber(totals.total)}</span>
           ...
           <aside className={`shrink-0 flex flex-col gap-4 ${panelEffectiveCollapsed ? "hidden" : ""}`} style={{ width: panelWidth, minWidth: panelWidth }}>
              ...
              <div className="rounded-md border border-border-strong bg-bg-surface p-4 shadow-sm shrink-0">
                  <h3 className="mb-3 text-[11px] font-black text-text-muted uppercase tracking-widest border-b pb-2 border-border-subtle flex items-center gap-2">
                    <Tag className="h-4 w-4 text-text-muted" /> ملخص الإجماليات
                  </h3>

                  <div className="space-y-2">
                    <div className="flex items-center justify-between py-1.5 px-2 rounded bg-bg-overlay">
                      <span className="text-2sm font-bold text-text-secondary">المجموع الفرعي</span>
                      <span className="text-sm font-black text-text-primary number-fmt">{formatNumber(totals.sub)}</span>
                    </div>
                   ...
                   <div className="flex flex-col gap-1.5 pr-2">
                    ...
                    <div className="flex items-center justify-between pt-2 border-t border-border-normal">
                      <span className="text-sm font-black text-text-primary">الإجمالي النهائي</span>
                      <span className="number-fmt-primary text-sm font-black text-emerald-700">{formatNumber(totals.total)}</span>
                    </div>
                 </div>
              </div>
              {/* Order Status */}
              <div className="rounded-md border border-border-strong bg-bg-surface p-5 shadow-md shrink-0">
                 <h3 className="mb-4 text-2sm font-black text-text-primary border-b pb-2 border-border-subtle uppercase tracking-widest flex items-center gap-2">
                   <Clock className="h-4 w-4 text-text-muted" /> حالة الطلب
                 </h3>
                 <div className="space-y-3">
                    ...
                    <p className="text-[11px] leading-relaxed text-text-muted text-center font-bold">
              ...
              {/* Save / Print */}
              <div className="rounded-md border border-border-normal bg-bg-surface p-4 shadow-sm shrink-0 flex flex-col gap-2">
                 ...
                 <PermissionGate page="purchase_orders" action="print">
                   <button onClick={() => setPrintPreview(true)} disabled={!lines.length}
                     className="w-full flex items-center justify-center gap-1.5 rounded-sm border border-border-normal bg-bg-surface px-3 py-2.5 text-[11px] font-bold text-text-secondary hover:border-emerald-300 hover:bg-bg-overlay transition-all disabled:opacity-40">
      ...
      <Modal open={warnModalOpen} onClose={() => setWarnModalOpen(false)} title="تحذير: مغادرة الصفحة" size="md" showDetach={false}>
        <div className="p-4 space-y-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              <h3 className="text-sm font-black text-text-primary mb-1">تجاهل التغييرات؟</h3>
          ...
          <div className="flex justify-end gap-2 pt-4 border-t border-border-subtle">
            <button onClick={() => setWarnModalOpen(false)} className="rounded-sm border border-border-strong bg-bg-surface px-5 py-2 text-sm font-black text-text-primary hover:bg-bg-overlay">
              تراجع وإكمال الطلب
            </button>
