...
function RecordChips({ counts }) {
  ...
  return (
    <div className="mt-1 flex flex-wrap gap-1">
      {entries.map(([k, v]) => (
        <span key={k} className="rounded-sm bg-bg-overlay px-1.5 py-0.5 text-[10px] font-bold text-text-secondary">
...
function SnapshotRow({ snap, onRestore, onExport, busy, perms }) {
  return (
    <div className="flex flex-col gap-2 rounded-sm border border-border-subtle bg-bg-surface p-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="min-w-0">
        <div className="flex flex-wrap items-center gap-2">
          <Clock className="h-3.5 w-3.5 text-text-muted" />
          <span className="text-xs font-black text-text-primary">{formatTime(snap.createdAt)}</span>
        ...
        {snap.label && <div className="mt-1 text-[11px] font-bold text-text-secondary">“{snap.label}”</div>}
        {snap.legacy && (
      ...
      <div className="flex shrink-0 gap-2">
        {perms.export && (
          <motion.button
            ...
            className="flex h-8 items-center gap-1 rounded-sm border border-border-normal px-3 text-[11px] font-black uppercase tracking-wider text-text-secondary transition-all hover:bg-bg-overlay active:scale-95 disabled:opacity-50"
          >
...
function DayNode({ year, month, day, onRestore, onExport, busy, perms }) {
  ...
  return (
    <div className="rounded-sm border border-border-normal bg-bg-overlay/40">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="flex w-full items-center justify-between gap-3 p-3 text-right transition-all hover:bg-bg-overlay"
      >
        <div className="flex items-center gap-2">
          <Calendar className="h-4 w-4 text-text-muted" />
          <span className="text-xs font-black text-text-primary">{`${day.day} ${monthLabel(month)} ${year}`}</span>
          <span className="rounded-sm bg-border-normal/70 px-1.5 py-0.5 text-[10px] font-black text-text-secondary">
  ...
  return (
    <div>
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        className="mb-2 flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-text-secondary"
      >
