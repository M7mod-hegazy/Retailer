...
function StatChip({ label, value }) {
  return (
    <div className="flex items-center justify-between rounded-sm border border-border-subtle bg-bg-overlay/60 px-3 py-2">
...
function SectionCard({ icon, title, children }) {
  return (
    <div className="rounded-sm border border-border-normal bg-bg-surface p-4">
      <div className="mb-3 flex items-center gap-2 text-[11px] font-black uppercase tracking-widest text-text-primary">
        {icon}
        {title}
      </div>
      {children}
        ...
        <div className="space-y-4">
          <div className="grid gap-4 md:grid-cols-2">
            <SectionCard icon={<Database className="h-3.5 w-3.5" />} title="بياناتك">
              <div className="grid grid-cols-2 gap-2">
                {counts.length === 0 && <div className="col-span-2 text-[11px] text-text-muted">لا توجد بيانات</div>}
                {counts.map(([key, val]) => (
              ...
              <div className="mt-3 text-[11px] font-bold text-text-secondary">
                حجم قاعدة البيانات: <span className="text-text-primary">{formatBytes(preview.db?.sizeBytes)}</span>
              </div>
            </SectionCard>
          ...
          <SectionCard icon={<FolderTree className="h-3.5 w-3.5" />} title="هذه النسخة">
            <div className="space-y-2">
              ...
              <div className="rounded-sm border border-border-subtle bg-bg-overlay/60 px-3 py-2">
                <div className="text-[11px] font-bold text-text-secondary">مكان الحفظ</div>
                <div className="mt-0.5 break-all text-[11px] font-bold text-text-primary ltr:text-left" dir="ltr">
                  {preview.targetDir}
                </div>
              </div>
              <label className="block">
                ...
                <input
                  ...
                  className="w-full rounded-sm border border-border-normal px-3 py-2 text-xs font-bold text-text-primary outline-none focus:border-slate-400"
                />
              </label>
            </div>
          </SectionCard>
