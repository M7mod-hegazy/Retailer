    ...
    <div className="relative" ref={containerRef}>
      <button 
        ...
        className={`flex items-center gap-2 h-10 px-3 rounded-xl border transition-all outline-none ${
          open 
            ? "bg-bg-surface border-amber-300 ring-4 ring-amber-500/10 shadow-sm" 
            ...
            <div className="max-h-[250px] overflow-y-auto no-scrollbar flex flex-col gap-0.5">
              <button
                ...
                className={`w-full text-right px-3 py-2 rounded-xl text-2sm font-bold transition-colors ${
                  value === "" ? "bg-amber-50 text-amber-700" : "text-text-secondary hover:bg-bg-overlay"
                }`}
              >
                {placeholder}
              </button>
              {options.map((opt) => (
                <button
                  ...
                  className={`w-full text-right px-3 py-2 rounded-xl text-2sm font-bold transition-colors ${
                    value === opt.value ? "bg-amber-50 text-amber-700" : "text-text-secondary hover:bg-bg-overlay"
                  }`}
                >
                  {opt.label}
                </button>
...
function InlineAddForm({ categories, onSubmit, saving, canBackdate, onAddCategoryClick, canAddCategory, lastAddedCategoryId }) {
  ...
  const fieldCls = "h-10 rounded-xl bg-bg-overlay/80 dark:bg-zinc-900/50 hover:bg-bg-overlay/80 dark:hover:bg-zinc-900/80 focus:bg-bg-surface dark:focus:bg-zinc-950 border border-border-normal/60 dark:border-zinc-800/60 focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 px-3.5 text-sm font-bold text-text-primary dark:text-zinc-200 outline-none placeholder:text-text-muted dark:placeholder:text-zinc-550 transition-all duration-200";
      ...
      <div className="p-1 flex flex-wrap items-end gap-3" dir="rtl">
        ...
        <div className="flex flex-col gap-1.5 shrink-0">
          ...
          <div className="relative">
            <select
              ...
              onKeyDown={e => { if (e.key === "Enter") noteRef.current?.focus(); }}
        ...
        <div className="flex-1 min-w-[260px] flex items-end gap-2">
          <div className="flex flex-col gap-1.5 flex-1">
            <label className="text-[10px] font-black text-slate-450 dark:text-zinc-500 uppercase tracking-wider px-0.5">المسحوب له / البيان</label>
            <input
              ref={noteRef} type="text" placeholder="لمن أو لماذا؟..." value={form.note}
          ...
          <button
            ...
            className="h-10 px-6 rounded-xl bg-primary hover:bg-primary-600 active:scale-95 text-white text-sm font-black disabled:opacity-40 transition-all flex items-center gap-2 whitespace-nowrap shrink-0 shadow-md shadow-amber-500/10"
          >
        ...
        <div className="p-8 flex flex-col gap-6">
          <div className="relative">
            <input 
              ...
              className="w-full bg-transparent text-5xl md:text-7xl number-fmt-primary text-zinc-900 placeholder:text-slate-200 outline-none pb-2 border-b-2 border-border-subtle focus:border-amber-500 transition-colors"
            />
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col gap-1.5">
              <div className="flex items-center justify-between">
                <label className="text-[11px] font-black text-text-muted uppercase tracking-widest">تصنيف السحب <span className="text-amber-500">*</span></label>
                {canAddCategory && (
                  <button
                    type="button"
                    onClick={onAddCategoryClick}
              ...
              <select 
                ...
                className="w-full h-12 bg-bg-overlay border border-border-normal rounded-xl px-4 text-sm font-bold outline-none focus:bg-bg-surface focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 appearance-none"
              >
          ...
          <div className="flex flex-col gap-1.5">
            <label className="text-[11px] font-black text-text-muted uppercase tracking-widest">البيان / المسحوب له</label>
            <input 
              ref={noteRef}
              ...
              className="w-full h-12 bg-bg-overlay border border-border-normal rounded-xl px-4 text-sm font-bold outline-none focus:bg-bg-surface focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10"
            />
          </div>
        </div>
  ...
  return (
    <div className="min-h-[100dvh] bg-[var(--bg-base)] flex flex-col font-sans w-full relative" dir="rtl">
      ...
      <div className="relative z-30 px-8 pt-12 pb-8 max-w-5xl mx-auto w-full">
        ...
        {/* ------------------------------------------------------------- */}
        {/* ------------------------------------------------------------- */}
        {/* NEW SMART COMMAND CENTER (Unified Console Deck) */}
        {/* ------------------------------------------------------------- */}
            ...
            <div className="relative flex flex-col rounded-[26px] bg-[var(--bg-surface)]/90 backdrop-blur-xl border border-[var(--border-normal)] shadow-[var(--shadow-elevated)] overflow-visible">
              ...
              <div className="px-5 pt-3 pb-2 flex flex-wrap items-center justify-between gap-3 border-b border-[var(--border-subtle)] bg-[var(--bg-surface)]/50">
                <div className="flex flex-wrap items-center gap-4">
                  <div className="flex items-center gap-1.5 cursor-default text-[12px]" title="إجمالي مسحوبات اليوم فقط">
                    <span className="h-2 w-2 rounded-full bg-amber-500 animate-pulse shrink-0" />
                    <span className="font-black text-text-muted dark:text-zinc-500 uppercase tracking-wider text-[10px]">سحب اليوم:</span>
                    <span className="font-black text-amber-600 font-mono">{fmt(stats.todayAmt)} ج.م</span>
                  </div>
              ...
              <div className="p-3 flex flex-col md:flex-row md:items-center gap-2.5">
                ...
                <div className="relative flex-1 w-full min-w-[200px]">
                  ...
                  <input
                    ...
                    className="w-full h-10 rounded-xl bg-bg-overlay/80 dark:bg-zinc-900/50 border border-transparent pr-10 pl-10 text-2sm font-bold text-zinc-805 dark:text-zinc-250 outline-none hover:bg-bg-overlay dark:hover:bg-zinc-900 focus:bg-bg-surface dark:focus:bg-zinc-950 focus:border-amber-400 focus:ring-4 focus:ring-amber-500/10 transition-all placeholder:text-slate-450 dark:placeholder:text-zinc-550" 
                  />
                  {query && (
                    <button onClick={() => setQuery("")} className="absolute left-3 top-1/2 -translate-y-1/2 p-0.5 rounded hover:bg-border-normal dark:hover:bg-zinc-700 text-text-muted hover:text-text-secondary">
            ...
            >
              ...
              <div className="flex flex-col gap-2">
                {dateRows.map(row => (
                  <div 
                    key={row.id} 
                    className="group relative bg-bg-surface rounded-3xl p-5 border border-border-subtle shadow-[0_4px_20px_-10px_rgba(0,0,0,0.05)] hover:shadow-md hover:border-border-normal transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                          ...
                          <div className="flex items-center gap-1 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                            ...
                            <PermissionGate page="withdrawals" action="delete">
                              {locked ? (
                                ...
                                <button onClick={(e) => { e.stopPropagation(); handleDelete(row.id); }} title="حذف" className="flex h-9 w-9 items-center justify-center rounded-xl bg-bg-overlay text-text-muted hover:text-amber-600 hover:bg-amber-50 transition-all">
