...
import { Users, Search, Plus, FileText, X, Phone, AlertTriangle, SlidersHorizontal, MessageSquare, LayoutList, Eye, ExternalLink, Printer, RefreshCw } from "lucide-react";
...
export default function CustomerAccountsPage() {
  ...
  const loadUrgencyMap = useCallback(async () => {
    try {
      const r = await api.get("/api/ajal-debts?party_type=customer&status=all&limit=500");
  ...
  // Load full invoice details for the detail modal
  useEffect(() => {
                ...
                <table className="w-full text-[12px] bg-white rounded-xl overflow-hidden shadow-sm">
                  ...
                  <tbody>
                    {tabData.map(inv => {
                      ...
                      const isFullyPaid = inv.status === "paid" || paid >= inv.total;
                      const isCancelled = inv.status === "cancelled";
                      return (
                      <tr key={inv.id} className="border-t border-slate-100 hover:bg-slate-50">
                        <td className="px-3 py-3 font-black font-mono text-blue-700 whitespace-nowrap">{inv.invoice_no || inv.doc_no || `#${inv.id}`}</td>
                        <td className="px-3 py-3">
                          <div className="flex items-center gap-1 flex-wrap">
                            <span className={`inline-flex items-center justify-center rounded-lg border px-2 py-0.5 text-[9px] font-black ${PTYPE_COLOR[inv.payment_type] || "text-slate-600 bg-slate-100 border-slate-200"}`}>
                              {arMethod(inv.payment_type)}
                            </span>
                            {inv.payment_type === "installments" && !inv.amended_by && (
                              <span className="px-1.5 py-0.5 rounded text-[9px] font-black bg-violet-100 text-violet-700 border border-violet-200">مقدم + قسط</span>
                            )}
                            {inv.payment_type === "credit" && !inv.amended_by && (
                          ...
                          {inv.payment_splits && (
                            <div className="flex flex-wrap gap-1 mt-1">
                              {inv.payment_splits.split("|||").map((split, i) => {
                                ...
                                const label = isCash ? "نقداً" : isCredit ? "آجل" : arMethod(methodKey);
                                return (
                        ...
                        <td className="px-3 py-3">
                          <span className={`rounded-full px-2 py-0.5 text-[10px] font-black ${inv.status === "paid" ? "bg-emerald-100 text-emerald-700" : inv.status === "cancelled" ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}>
                            {inv.status === "paid" ? "مسدد" : inv.status === "cancelled" ? "ملغي" : inv.status === "partial" ? "مدفوع جزئياً" : inv.status || "مكتمل"}
                          </span>
                        </td>
                        <td className="px-3 py-3">
        ...
        <Modal onClose={() => { setDetailInvoice(null); setDetailData(null); }} width="640px">
          <div className="p-5">
            <div className="flex items-center justify-between mb-4">
              ...
              <button onClick={() => { setDetailInvoice(null); setDetailData(null); }} className="h-8 w-8 flex items-center justify-center rounded-lg bg-slate-100 text-slate-400 hover:text-zinc-900 transition-colors">
                <X className="h-4 w-4" />
              </button>
            </div>
            {detailLoading ? (
              ...
              <>
                ...
                {(() => {
                  ...
                  const mPaid = Number(detailData.amount_received || 0);
                ...
                <div className="rounded-xl border border-slate-200 overflow-hidden mb-4">
                  ...
                  <div className="bg-slate-900 text-white px-3 py-3">
                    ...
                    {Number(detailData.increase) > 0 && <div className="flex justify-between text-[10px] mb-1"><span className="text-slate-400">الإضافة</span><span className="font-mono text-emerald-300">+ {fmt(detailData.increase)}</span></div>}
                  ...
                  <div className="rounded-xl border border-slate-200 p-4 mb-4">
                    ...
                    <div className="flex flex-col gap-1.5">
                      {detailData.payments.map((p, i) => {
                        ...
                        return (
                          <div key={i} className={`flex items-center justify-between rounded-lg px-3 py-2 ${isCash ? "bg-emerald-50 border border-emerald-200" : isCredit ? "bg-amber-50 border border-amber-200" : "bg-slate-50 border border-slate-200"}`}>
                            <div className="flex items-center gap-1.5">
                              <span className={`text-[11px] font-black ${isCash ? "text-emerald-700" : isCredit ? "text-amber-700" : "text-slate-700"}`}>{p.method_name || arMethod(p.method)}</span>
                              {!isCash && <span className="text-[8px] font-bold text-slate-400 bg-slate-200/50 px-1.5 py-0.5 rounded">لا يؤثر على الخزنة</span>}
                            </div>
                ...
                <div className="flex gap-2 mt-4">
                  <button onClick={() => window.open(`/invoices/${detailInvoice.id}`, "_blank")}
                    className="flex-1 flex items-center justify-center gap-2 rounded-xl bg-blue-600 py-2.5 text-[12px] font-black text-white hover:bg-blue-700 transition-colors">
