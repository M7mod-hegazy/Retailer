...
export default function AccountImportHistoryTab({ entityType }) {
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-12 text-2sm font-bold text-slate-500">
  ...
  if (!batches.length) {
    return (
      <div className="py-12 text-center text-2sm font-bold text-slate-400">
            ...
            {batches.map((b) => {
              ...
              return (
                <tr key={b.id} className="transition-colors hover:bg-slate-50/40 text-slate-700 font-semibold">
                  <td className="px-6 py-4 font-black text-slate-900 truncate max-w-[240px]" title={b.file_name || `#${b.id}`}>
                    {b.file_name || `#${b.id}`}
                  </td>
