...
export default function AccountImportHistoryTab({ entityType }) {
  const apiBase = entityType === "customers" ? "/api/customers" : "/api/suppliers";
  const canUndo = usePermission(entityType === "customers" ? "customer_accounts" : "supplier_accounts", "import_undo");
  ...
  const load = useCallback(async () => {
    setLoading(true);
    try {
      const res = await api.get(`${apiBase}/import/batches`);
  ...
  const download = async (batch) => {
    try {
      const res = await api.get(`${apiBase}/import/batches/${batch.id}/file`, { responseType: "blob" });
  ...
  const handleUndoConfirm = async () => {
    ...
    try {
      ...
      else if (body?.reason === "activity") toast.error("لا يمكن التراجع: هناك معاملات مرتبطة بهذه الحسابات.");
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center gap-2 py-12 text-2sm font-bold text-text-secondary">
  ...
  if (!batches.length) {
    return (
      <div className="py-12 text-center text-2sm font-bold text-text-muted">
            ...
            {batches.map((b) => {
              ...
              return (
                <tr key={b.id} className="transition-colors hover:bg-bg-overlay/40 text-text-primary font-semibold">
                  <td className="px-6 py-4 font-black text-text-primary truncate max-w-[240px]" title={b.file_name || `#${b.id}`}>
                    {b.file_name || `#${b.id}`}
                  </td>
